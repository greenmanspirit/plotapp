...
$(document).ready(function() {
	...
	$('#addVirtueLink').click(function() {
		...
		$('#virtueListForm').append(newCharFormElement(num, 'virtue'));
		return false;
	});
	$('#addViceLink').click(function() {
		var num  = $('.vice').length;
		$('#viceListForm').append(newCharFormElement(num, 'vice'));
		return false;
	});
	$('#addHabitLink').click(function() {
		var num  = $('.habit').length;
		$('#habitListForm').append(newCharFormElement(num, 'habit'));
		return false;
	});
});
function newCharFormElement(idNum, charAttribute) {
	...
	var text = $('<input>').attr('type','text').attr('name',arrayRef+'[detail]').attr('id',attrStr+'detail').addClass(charAttribute).addClass('width85').attr('size',30);
